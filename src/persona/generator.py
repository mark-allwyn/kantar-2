...
class PersonaGenerator:
    ...
    def __init__(self, random_seed: Optional[int] = None):
        ...
        # Define distributions based on questionnaire
    ...
    def _define_distributions(self) -> Dict[str, Any]:
        ...
        return {
            'gender': {
                ...
                'weights': [0.45, 0.45, 0.05, 0.03, 0.02]  # Approximate distribution
            },
            'age': {
                'min': 18,
                'max': 75,
                ...
                'band_weights': [0.33, 0.34, 0.33]  # Equal distribution across bands
            },
            'occupation': {
                # Exclude screened occupations
                'values': [
            ...
            'category_buyer': {
                ...
                'weights': [0.3, 0.2, 0.3, 0.2],  # Approximate
                'multi_select': True,
                'max_selections': 3
            },
            'category_non_rejector': {
                # S5: What would you NEVER spend money on (same options as S4)
                # If they select "Paper scratchcards" here -> screen out
                'values': [
                    'Lottery played in-store/person',
                    'Lottery played online/app',
                    'Paper scratchcards bought in person',
                ...
                'weights': [0.1, 0.1, 0.05, 0.75],  # Most people don't reject, low chance of rejecting paper SC
                'multi_select': True,
                'max_selections': 3
            },
            'sc_players': {
            ...
            'inertia': {
                'min': 1,
                'max': 7,
                'distribution': 'normal',  # Normal distribution around midpoint
                'mean': 4,
                'std': 1.5
            },
            'brand_buyers': {
                ...
                'multi_select': True,
                'max_selections': 5  # Can buy multiple brands
            }
        }
    ...
    def _sample_age(self) -> int:
        ...
        # Sample band first (for quota control)
    ...
    def _sample_brand_buyers(self) -> List[str]:
        ...
        values = dist['values']
        weights = dist['weights']
        max_selections = dist['max_selections']

        # Decide number of brands to select (1-5)
